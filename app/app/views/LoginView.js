...
const _user = require('../lib/methods/user');
import  Server  from '../ServerConfig';
import {RocketChat_user} from '../lib/rocketchat';
...
class LoginView extends React.Component {
	...
	submit = async() =>  {
		...
		let meUserinfo = new _user();
		let res = await meUserinfo.login(user,password);
		storage.save('userInfo', meUserinfo.userInfo);
		if (res.err == null){
			Keyboard.dismiss();
		...
		// http.post(`${Server.ChatIp}:${Server.APIPort}/login`, {
		// 	account: user,
		// 	password: password,
		// }).then((res) => {
		// 	let result = res.data;
		// 	if (!result.code) {
		// 		//登录成功
		// 		http.setUserToken(result.data._user_id, result.data._token);
		// 		//火箭
		// 		Keyboard.dismiss();
		// 		loginRequest({ user, password, code });
		// 		analytics().logEvent('login');;
