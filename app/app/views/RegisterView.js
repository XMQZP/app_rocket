...
const http = require('../lib/methods/subscriptions/http');
...
class RegisterView extends React.Component {
	...
	submit = async() => {
		...
		const { loginRequest } = this.props;
		 ...
		 let res = await user.register(username,password,email,name);
		if(res.err == null){
			loginRequest({ user: email, password });
		}else {
			Alert.alert('注册失败');
		}
		// http.post(`${Server.ChatIp}:${Server.APIPort}/register_user`, {
		// 	account: username,
		// 	password: password,
		// 	email:email,
		// 	name:name,
