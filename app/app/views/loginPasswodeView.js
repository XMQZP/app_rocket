...
class loginPasswodeView extends React.Component {
				...
				>
					...
					<View style={[{marginTop:20,flexDirection:'row'}]}>
						<Button
							title={'确认修改'}
							onPress={ async() => {
								Alert.alert(`${this.data[0].value}/${this.data[1].value}/${this.data[2].value}`)
	...
	async HttpRequest() {
		let res = await user.cheatPassword(this.data[0].value, this.data[1].value);
		if (res.err == null) {
			Alert.alert('添加成功');
...
export default connect(mapStateToProps, mapDispatchToProps)(withTheme(loginPasswodeView));
